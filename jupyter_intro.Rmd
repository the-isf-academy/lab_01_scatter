...
Now you can use the `top_toppings` and `topping_rank` function anywhere in the notebook! 💻 **Run the code block below to see them in action.** 💻
...
Make sure you include `%autosave 0` in the first cell of all your notebooks. Juypter has an autosaving feature, but we will use manual saves to make sure we alwasy have synced `.ipynb` and `.Rmb` files.
